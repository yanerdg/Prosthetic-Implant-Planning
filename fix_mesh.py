...
def remove_duplicate_vertices(vertices, faces, epsilon=1e-4):
    """
    手动实现顶点去重，同时保持面索引有效。
    返回: (unique_vertices, new_faces)
    """
    ...
    # 将顶点四舍五入到指定精度，用于判定重复点
    rounded_vertices = np.round(vertices / epsilon) * epsilon
    ...
    # === 保证 new_faces 为 (N,3) numpy 数组 ===
    new_faces = np.asarray(new_faces, dtype=np.int32)
    if new_faces.ndim == 2 and new_faces.shape[1] == 1:
        # 如果意外嵌套（每行一个 array），强制展开
        new_faces = np.vstack(new_faces[:, 0])
    elif new_faces.ndim > 2:
...
def getMesh(objMeshPath):
    ...
    v, f = igl.read_triangle_mesh(objMeshPath)
    return v, f
...
def getCoarsePatch(v_bd_id_init, v, f):
    ...
    while v_bd_id.size > 3:
        ...
        if edgeRatio <= 1.0: # v_bd[min_angle_index] #边界上与左右顶点夹角最小的点
            ...
            temp_new_vs = []
            temp_new_v_ids = []
            if not np.isfinite(edgeRatio) or edgeRatio < 1:
                print(f"[Warning] edgeRatio invalid ({edgeRatio}), reset to 1.0 at vertex {v_bd_id_c}")
                edgeRatio = 1.0

            edgeRatio_int = int(round(edgeRatio))
            edgeRatio_int = max(edgeRatio_int, 1)  # 至少为1，避免 range(1,0)
...
def uniform_laplacian_smoothing(v, f, iterations=1):
    """使用均匀拉普拉斯（uniform Laplacian）平滑"""
    for _ in range(iterations):
        adj = igl.adjacency_matrix(f)
        deg = np.array(adj.sum(axis=1)).flatten()
        deg[deg == 0] = 1
        v_new = (adj @ v) / deg[:, None]
        v = v_new
    return v
@ray.remote
def generateOneRefinedPatchedTooth(srcDstFileTuple, keepPrev=False, voxel_size=0.2):
    ...
    while v_bd_id.size > 0:
        ...
        if patchV.size > 0:
            ...
            v, f = remove_duplicate_vertices(v, f, epsilon=0.1)


        # refinedPatchV = refineMeshByFirstOrderLaplacian(patchF, patchV_id, v_bd_id, v)
        v_bd_id = igl.boundary_loop(f)
    #v = igl.per_vertex_attribute_smoothing(v, f) # 整体 uniform laplacian smoothing
    v = uniform_laplacian_smoothing(v, f) # 整体 uniform laplacian smoothing
    # 对三角网格中较大的三角形进行上采样
...
def getSrcDstFilePathList(srcObjRoot, dstObjRootDir, dstTxtRootDir, teethIndexRepairList):
    """
    遍历患者文件夹下的牙齿obj文件，并在输出文件夹中保留相同的目录结构。
    """
    ...
    # 遍历每个患者文件夹
    for patient_dir in os.listdir(srcObjRoot):
        patient_path = os.path.join(srcObjRoot, patient_dir)
        if not os.path.isdir(patient_path):
            continue  # 跳过非目录文件

        # 创建对应输出路径
        dst_patient_obj_dir = os.path.join(dstObjRootDir, patient_dir)
        dst_patient_txt_dir = os.path.join(dstTxtRootDir, patient_dir)
        os.makedirs(dst_patient_obj_dir, exist_ok=True)
        os.makedirs(dst_patient_txt_dir, exist_ok=True)

        # 遍历当前患者下的牙齿obj文件
        for file in os.listdir(patient_path):
            if not file.endswith(".obj"):
                continue
            tooth_index = os.path.splitext(file)[0]
            if tooth_index not in teethIndexRepairList:
                continue

            srcObjFile = os.path.join(patient_path, file)
            dstObjFile = os.path.join(dst_patient_obj_dir, file)
            dstTxtFile = os.path.join(dst_patient_txt_dir, tooth_index + ".txt")

            # 检查目标obj文件是否已经存在，如果存在则跳过
            if not os.path.exists(dstObjFile):
                srcDstTupleList.append((srcObjFile, dstObjFile, dstTxtFile))
            else:
                print(f"目标文件已存在，跳过: {dstObjFile}")
...
if __name__ == "__main__":
    ...
    srcObjRootDir = "F:/ColorOralSegDataset/eachtooth_obj/"
    dstObjRootDir = "F:/ColorOralSegDataset/repaired_obj/"
    dstTxtRootDir = "F:/ColorOralSegDataset/repaired_txt/"

    teethIndexRepairList = [
        "11","12","21","22","13","14","23","24","33","34","43","44",
        "15","16","17","25","26","27","35","36","37","45","46","47",
        "18","28","38","48"
    ]

    FEASIBLE_COS_THRE = 0.6  # 可根据牙位修改

    # teethIndexRepairList = ["31", "32", "41", "42"]
    # FEASIBLE_COS_THRE = 0.8  # TOOTH 31,32,41,42
    srcDstTupleList = getSrcDstFilePathList(
        srcObjRootDir, dstObjRootDir, dstTxtRootDir, teethIndexRepairList
    )

    keep_prev_flag = False  # 是否保留已有数据
    voxel_size = 0.2        # 体素下采样大小

    num_cpus = psutil.cpu_count(logical=False)
    ray.init(num_cpus=num_cpus)

    ray.get([
        generateOneRefinedPatchedTooth.remote(srcDstFs, keep_prev_flag, voxel_size)
        for srcDstFs in srcDstTupleList
    ])

    #SINGLE TEST
    # srcObjRootDir = r"F:\ColorOralSegDataset\oral_obj"
    # dstObjRootDir = "F:/ColorOralSegDataset/test/"
    # dstTxtRootDir = "F:/ColorOralSegDataset/test/"
    # fName = "1/1_lower.obj"
